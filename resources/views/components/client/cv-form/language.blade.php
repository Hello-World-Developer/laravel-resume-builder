...
<div class="mt-4" id="language">
    <input class="peer/show-language hidden" id="show-language" type="checkbox" checked>
    ...
    <div class="hidden peer-checked/show-language:block">
        <div id="language-container">
            <div class="border mt-4 p-4" id="language_{{ $id }}">
                <div class="">
                    <label class="text-md text-gray-500 inline-block mb-1" for="language_name_{{ $id }}">
                        language
                    </label>
                    <input class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                           id="language_name_{{ $id }}" name="languages[{{ $id }}][name]"
                           type="text" onkeyup="handlelanguage(this)">
                </div>
                <div class="flex mt-3">
                    <div class="flex-1">
                        <label class="block mb-1 text-sm text-md text-gray-500 dark:text-white" for="default-range">
                            Level</label>
                        <div class="flex items-center w-full">
                            <input class="language-range flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
                                   id="language_range_{{ $id }}" name="languages[{{ $id }}][level]"
                                   type="range" value="0" onchange="handlelanguage(this)">
                ...
                <div class="flex mt-5 justify-end">
                    <x-common.button.outline-dark-icon data-target="#language_{{ $id }}"
                                                       onclick="removelanguageField(this.dataset.target)">
                        <x-icon.trash />
        ...
        <x-common.button.outline-dark class="flex items-center mt-5" onclick="addlanguage()">
